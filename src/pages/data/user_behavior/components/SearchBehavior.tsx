import { querySearchBehavior } from '@/services/BehaviorService';
...
const SearchTable: React.FC = () => {
  ...
  const edit = (record: Partial<Item>) => {
    form.setFieldsValue({ SearchTarget: '', ResultsCount: '', ...record });
  ...
  const formattedArray = (value: any) => {
    const newArr = value.map(function (arr) {
      const date = new Date(arr.search_time);
      const formattedDate =
        date.getFullYear() +
        '-' +
  ...
  const columns = [
    ...
    },
    {
      ...
      // width: '20%',
      ...getColumnSearchProps('search_time'),
      sorter: (a, b) => new Date(a.search_time) - new Date(b.search_time),
    },
    {
      title: 'SearchTarget',
      ...
      // width: '20%',
      editable: true,
      ...getColumnSearchProps('search_query'),
    },
    {
      title: 'ResultsCount',
