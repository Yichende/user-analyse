import { queryCommentBehavior } from '@/services/BehaviorService';
...
const CommentTable: React.FC = () => {
  ...
  const edit = (record: Partial<Item>) => {
    form.setFieldsValue({ CommentTarget: '', Rating: '', Comment: '', ...record });
  ...
  const formattedArray = (value: any) => {
    const newArr = value.map(function (arr) {
      const date = new Date(arr.comment_time);
      const formattedDate =
        date.getFullYear() +
        '-' +
  ...
  const columns = [
    ...
    },
    {
      ...
      // width: '20%',
      ...getColumnSearchProps('comment_time'),
      sorter: (a, b) => new Date(a.comment_time) - new Date(b.comment_time),
    },
    {
      title: 'CommentTarget',
      dataIndex: 'comment_target',
      // width: '20%',
      editable: true,
      ...getColumnSearchProps('comment_target'),
    },
    {
      title: 'Rating',
      dataIndex: 'rating',
