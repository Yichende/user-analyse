import { queryPurchaseBehavior } from '@/services/BehaviorService';
...
const PurchaseTable: React.FC = () => {
  ...
  const formattedArray = (value: any) => {
    const newArr = value.map(function (arr) {
      const date = new Date(arr.purchase_time);
      const formattedDate =
        date.getFullYear() +
        '-' +
  ...
  const columns = [
    ...
    },
    {
      ...
      // width: '20%',
      ...getColumnSearchProps('purchase_time'),
      sorter: (a, b) => new Date(a.purchase_time) - new Date(b.purchase_time),
    },
    {
      title: 'PurchaseProduct',
      dataIndex: 'purchased_product',
      // width: '20%',
      editable: true,
      ...getColumnSearchProps('purchased_product'),
    },
    {
      title: 'Quantity',
      dataIndex: 'quantity',
